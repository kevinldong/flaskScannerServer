...
def main():
    ...
    parser.add_argument('action', type=str, choices=['upload', 'download', 'u', 'd'],help="whether to upload or download a file")
    ...
    # Setup variables
    # TODO: make these configuarable via optional command line args
    ...
    if args.action == 'download' or args.action== 'd':
        ...
        r = session.get(f"{url}/download/{args.file}")
        if r.status_code != 200:
